 ...
 * 
 */
@Name(AutomaticOneEventsTreeHandler.NAME)
@Scope(ScopeType.PAGE)
@Install(precedence = Install.FRAMEWORK)
@BypassInterceptors
public class AutomaticOneEventsTreeHandler extends AbstractTreeHandler<Evento> {

	public static final String LISTA_EVENTO_VARIABLE = "listaEvento";
	private static final LogProvider log = Logging.getLogProvider(AutomaticOneEventsTreeHandler.class);
	private static final long serialVersionUID = 1L;

	public static final String NAME = "automaticOneEventsTree";
	...
	private String agrupamentosInstance;

	public static AutomaticOneEventsTreeHandler instance() {
		return (AutomaticOneEventsTreeHandler) org.jboss.seam.Component.getInstance(AutomaticOneEventsTreeHandler.NAME);
	}
	...
	public List<EntityNode<Evento>> getRoots(String agrupamentos) {
		...
		if (this.agrupamentosInstance == null || !this.agrupamentosInstance.equals(agrupamentos)) {
			if (idEventos == null) {
	...
	protected String getQueryChildren() {
		return "select n from Evento n where n.eventoSuperior = :" + EntityNode.PARENT_NODE;
	}
	...
	protected String getIdEventos(String agrupamentos) {
		...
		if (agrupamentos != null && !agrupamentos.trim().isEmpty()) {
	...
	public void setSelected(Evento selected, boolean isMultiplo) {
		this.setExibeMsg(false);
		if(getEventoBeanList() != null && getEventoBeanList().size() > 0){
			FacesMessages.instance().add("Não é permitido selecionar mais de um evento!");
			this.setExibeMsg(true);
			return;
		}
