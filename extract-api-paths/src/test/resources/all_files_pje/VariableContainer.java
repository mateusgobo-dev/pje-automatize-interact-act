...
public abstract class VariableContainer implements Serializable {
	 ...
	 * {@link #setVariables(Map)} is the same as this method, but it was added
	 ...
	 * variables. This method is the same as {@link #addVariables(Map)} and this
	 * method was added for naming consistency.
	 */
	...
	public Map<String, Object> getVariablesLocally() {
		...
		List<String> garbage = new ArrayList<String>(0);
		if (variableInstances != null) {
			for (Map.Entry<String, VariableInstance> entry : variableInstances.entrySet()) {
				...
				if (!variables.containsKey(name)) {
					...
					} catch (JbpmException|NullPointerException e) {
						garbage.add(name);
						log.error("VARIAVEL COM ERRO DE SERIALIZACAO ====>>> " + name + ":" + e.getLocalizedMessage());
					}
				}
			}
			for (String name : garbage) {
				deleteVariableLocally(name);
			}
		}
		return variables;
