...
public final class DefaultFaceletFactory extends FaceletFactory {
	...
	private DefaultFacelet createFacelet(URL url) throws IOException, FaceletException, FacesException, ELException {
		...
		try {
			// ========== Adicionado para logar demora do facelets na
			// inicialização
			Field field = Reflections.getField(this.compiler.getClass(), "initialized");
			if (field != null) {
				try {
					Boolean init = (Boolean) Reflections.get(field, this.compiler);
					if (init.equals(Boolean.FALSE)) {
						log.warning("Initializing facelets compiler");
					}
				} catch (Exception e) {
				}
			}
