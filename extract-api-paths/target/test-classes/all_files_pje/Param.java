...
public final class Param {
	...
	/**
	 * Get the current value of a page or redirection parameter from the model,
	...
	public String getStringValueFromModel(FacesContext facesContext) {
		...
		if (value == null) {
			...
			return converter == null ? value.toString() : converter.getAsString(facesContext,
	...
	public String getStringValueFromRequest(FacesContext facesContext, Map<String, String[]> requestParameters)
		...
		if (parameterValues == null || parameterValues.length == 0) {
			// Page parameters are propagated in the page context through faces
			// requests
		...
		// if the length is zero and the parameter is required, treat the value
		// as
		// missing (and report it as null); this differs from how JSF handles
		// inputs
		if (value.length() == 0) {
			if (isRequired()) {
				addRequiredMessage(facesContext);
	...
	public Object convertValueFromString(FacesContext facesContext, String value) {
		...
		return converter == null ? value : converter.getAsObject(facesContext, facesContext.getViewRoot(), value);
	}

	/**
	 * Validate the pre-converted value of the parameter using the JSF validator
	...
	public void validateConvertedValue(FacesContext facesContext, Object value) {
		...
		if (valueExpression != null && validateModel) {
			...
			try {
				invalidValues = Validators.instance().validate(valueExpression.toUnifiedValueExpression(), elContext,
						value);
			...
			// ITX - Changed for NullPointerException
	...
	private FacesMessage createMessage(InvalidValue[] invalidValues) {
		return FacesMessages.createFacesMessage(FacesMessage.SEVERITY_ERROR, INVALID_MESSAGE_ID, "'" + name
				+ "' parameter is invalid: " + invalidValues[0].getMessage(), new Object[] { invalidValues[0], name });
	}

	private FacesMessage createMessage(Throwable cause) {
		return FacesMessages.createFacesMessage(FacesMessage.SEVERITY_ERROR, INVALID_MESSAGE_ID, "'" + name
				+ "' parameter is invalid: " + cause.getMessage(), new Object[] { cause.getMessage(), name });
	}

	private void addRequiredMessage(FacesContext facesContext) {
		throw new ValidatorException(FacesMessages.createFacesMessage(FacesMessage.SEVERITY_ERROR, REQUIRED_MESSAGE_ID,
				"'" + name + "' parameter is required", name));
	}
