...
public class ResteasyBootstrap {
	...
	public void init() {
		...
		AnnotationDeploymentHandler handler = (AnnotationDeploymentHandler) deployment.getDeploymentHandlers()
				.get(AnnotationDeploymentHandler.NAME);
...
//		getDispatcher().setMediaTypeMappings(application.getMediaTypeMappings());
	...
	protected Collection<Class<?>> findProviders(AnnotationDeploymentHandler handler) {
		return findTypes(handler, application.isScanProviders(), javax.ws.rs.ext.Provider.class.getName(),
				application.getProviderClassNames());
	}

	protected Collection<Class<?>> findResources(AnnotationDeploymentHandler handler) {
		return findTypes(handler, application.isScanResources(), javax.ws.rs.Path.class.getName(),
	...
	protected Collection<Component> findSeamComponents() {
		...
		for (String applicationContextName : applicationContextNames) {
			if (applicationContextName.endsWith(".component")) {
				Component seamComponent = (Component) Component.getInstance(applicationContextName,
	...
	protected void registerProviders(Collection<Component> seamComponents, Collection<Class<?>> providerClasses) {
		 ...
		 * !seamComponent.getBeanClass().isAnnotationPresent(javax.ws.rs.ext.
		 * Provider.class)) { break; }
		 * 
		 ...
		 * (StringConverter.class.isAssignableFrom(seamComponent.getBeanClass())
	...
	protected void registerResources(Collection<Component> seamComponents, Collection<Class<?>> resourceClasses) {
		...
		for (Component seamComponent : seamComponents) {
			...
			case JAVA_BEAN:
				...
				case SESSION:
					...
					break;
				default:
					throw new RuntimeException("Resource Seam component '" + seamComponent.getName()
							+ "' must be scoped either " + "EVENT, APPLICATION, STATELESS, or SESSION");
				}
				break;
			}
		...
		for (Class<?> resourceClass : resourceClasses) {
			...
			// components.xml
				...
				throw new RuntimeException("Only stateless EJBs can be JAX-RS resources, remove from configuration: "
	...
	protected void registerHomeQueryResources(Component seamComponent) {
		...
		if (path != null) {
			...
			log.debug("registering resource, configured ResourceHome/Query on path {1}, as Seam component: {0}",
					seamComponent.getName(), path);

			ResourceFactory factory = new SeamResteasyResourceFactory(seamComponent.getBeanClass(), seamComponent,
	...
	protected void registerInterfaceSeamComponentResource(Component seamComponent, Class resourceInterface) {
		log.debug("registering resource, annotated interface {1}, as Seam component: {0}", seamComponent.getName(),
				resourceInterface.getName());

		ResourceFactory factory = new SeamResteasyResourceFactory(resourceInterface, seamComponent,
				getDispatcher().getProviderFactory());
	...
	protected void registerStatelessEJBResource(Class ejbImplementationClass) {
		...
		log.debug("registering resource, stateless EJB implementation {1}, as RESTEasy JNDI resource name: {0}",
				jndiName, ejbImplementationClass.getName());
	...
	protected void registerSeamComponentProvider(Component seamComponent) {
		...
		getDispatcher().getProviderFactory().registerProviderInstance(Component.getInstance(seamComponent.getName()));
	}
			...
			if (anInterface.isAnnotationPresent(annotation)) {
				if (resourceInterface != null) {
					throw new IllegalStateException(
							"Only one business interface can be annotated " + annotation + ": " + seamComponent);
				}
