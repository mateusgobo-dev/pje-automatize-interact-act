...
@Name(RemanescentesList.NAME)
@BypassInterceptors
@Scope(ScopeType.PAGE)
public class RemanescentesList extends FiltrosPautaJulgamentoList<ProcessoTrf> {

	public static final String NAME = "remanescentesList";
	
	private static final long serialVersionUID = 1L;
	
	private static final String DEFAULT_EJBQL = FiltrosPautaJulgamentoList.DEFAULT_EJBQL +
												"where o.orgaoJulgador in " +
													"(select cs.orgaoJulgador from ComposicaoSessao cs " +
														"where cs.sessao = #{pautaJulgamentoAction.sessao}) and " +
													"exists (select ps from PautaSessao ps where " +
														"ps.processoTrf = o and ps.sessao.situacaoSessao in ('E','F') and " +
														"ps.tipoSituacaoPauta.classificacao in ('D', 'R') and " +
														"ps.dataSituacaoPauta >= (current_date - o.orgaoJulgadorColegiado.diaRetiradaAdiada) and " +
