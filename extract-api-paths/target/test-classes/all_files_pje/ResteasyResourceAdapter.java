   ...
   public String getResourcePath()
   {
	...
	public void getResource(final HttpServletRequest request, final HttpServletResponse response)
		...
		try {
			...
			// TODO: As far as I can tell from tracing RE code: All this thread-local stuff
			// has no effect because
			// the "default" provider factory is always used. But we do it anyway, just to
			// mimic the servlet handler
			// in RE...
					...
					try {
						...
						if (identity != null && 
							...
							KeycloakSecurityContext ksc = (KeycloakSecurityContext) request
									.getAttribute(Constantes.SSO_CONTEXT_NAME);
							
							Integer idUsuarioLocalizacao = null;
							
							if(!StringUtil.isEmpty(request.getHeader("X-pje-usuario-localizacao")) &&
									StringUtil.isNumeric(request.getHeader("X-pje-usuario-localizacao"))) {
								idUsuarioLocalizacao = Integer.parseInt(request.getHeader("X-pje-usuario-localizacao"));
							}
							
							processAuthentication(ksc, identity, idUsuarioLocalizacao);
						}
						...
						HttpRequest in = new HttpServletInputMessage(request, response, null, theResponse, headers,
						 ...
						 * 
						 * session.isNew() check - do not close non-anemic sessions established by the
						 * view layer (JSF) which are reused by the JAX-RS requests (so that the
						 * requests do not have to be re-authorized)
						 */
						if (Authenticator.instance().isBearerTokenAuthentication()
								|| (application.isDestroySessionAfterRequest() && request.getSession().isNew())) {
	...
	private void processAuthentication(KeycloakSecurityContext ksc, Identity identity, Integer idUsuarioLocalizacao) {
		if(ksc != null && !identity.isLoggedIn() && !StringUtil.isEmpty((String)ksc.getToken().getPreferredUsername())) {
			Authenticator.instance().authenticateSSO(false, ksc, idUsuarioLocalizacao);
		}
	}
