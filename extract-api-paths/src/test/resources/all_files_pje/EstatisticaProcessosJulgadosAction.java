...
import br.com.infox.pje.bean.EstatisticaProcessosJulgadosBean;
import br.com.infox.pje.bean.EstatisticaProcessosJulgadosSubTableBean;
import br.com.infox.pje.list.EstatisticaProcessosJulgadosList;
...
/**
 * Classe action controladora do listView de /EstatisticaProcesso/Julgados/
 * 
 * @author Luiz Carlos Menezes
 * 
 */
@Name(value = EstatisticaProcessosJulgadosAction.NAME)
@Scope(ScopeType.CONVERSATION)
public class EstatisticaProcessosJulgadosAction implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 6953326453257532837L;

	public static final String NAME = "estatisticaProcessosJulgadosAction";

	private EstatisticaProcessosJulgadosList estatisticaProcessosJulgadosList = new EstatisticaProcessosJulgadosList();
	private static final String TEMPLATE_XLS_PATH = "/EstatisticaProcesso/Julgados/processosJulgadosTemplate.xls";
	...
	private List<EstatisticaProcessosJulgadosBean> estatisticaBeanList;
	...
	public SecaoJudiciariaManager getSecaoJudiciariaManager() {
		return secaoJudiciariaManager;
	}

	public void setSecaoJudiciariaManager(SecaoJudiciariaManager secaoJudiciariaManager) {
		this.secaoJudiciariaManager = secaoJudiciariaManager;
	}
	 ...
	 * EstatisticaDistribuidosJulgadosBean.java
	 * 
	 * @return lista com os dados para Distribuídos/Julgados
	 */
	public List<EstatisticaProcessosJulgadosBean> estatisticaProcessosJulgadosList() {
		if (estatisticaBeanList == null) {
			...
			estatisticaBeanList = buildEstatisticaProcessosJulgadosList();
			for (EstatisticaProcessosJulgadosBean obj : estatisticaBeanList) {
				for (int i = 0; i < 12; i++) {
	...
	private List<EstatisticaProcessosJulgadosBean> buildEstatisticaProcessosJulgadosList() {
		List<Map<String, Object>> mapList = getEstatisticaProcessosJulgadosList().getResultList();
		List<EstatisticaProcessosJulgadosBean> estatisticaProcessosJulgadosBeanList = new ArrayList<EstatisticaProcessosJulgadosBean>();
		EstatisticaProcessosJulgadosBean bean = new EstatisticaProcessosJulgadosBean();
		for (Map<String, Object> map : mapList) {
			if (bean.getCodEstado() == null) {
				...
				bean = new EstatisticaProcessosJulgadosBean();
				bean.setCodEstado(map.get("codEstado").toString());
				if (!ParametroUtil.instance().isPrimeiroGrau()) {
					if (historicoEstatisticaEventoProcessoManager.getDataAtualizacaoSessao(bean.getCodEstado()) != null) {
			...
			bean.setSubList(estatisticaEventoProcessoManager.buscaListaVarasSecaoJulgados(bean.getCodEstado(),
					dataInicioFormatada, dataFimFormatada));
			for (EstatisticaProcessosJulgadosSubTableBean obj : bean.getSubList()) {
			...
			bean.getSubList().addAll(ordenarVaras(lista));
			estatisticaProcessosJulgadosBeanList.add(bean);
			totalGeral += bean.getTotalJulgados();
		}
		...
		return estatisticaProcessosJulgadosBeanList;
	}
	...
	public void gravarLogRelatorio() {
		...
		if (estatisticaProcessosJulgadosList().size() > 0) {
			RelatorioLog relatorioLog = new RelatorioLog();
			relatorioLog.setDataSolicitacao(new Date());
			relatorioLog.setDescricao("Estatística de Processos Julgados");
	 ...
	 */
	public void exportarProcessosJulgadosXLS() {
		try {
			if (estatisticaBeanList.size() > 0) {
				exportarXLS(TEMPLATE_XLS_PATH, "ProcessosJulgados.xls", "estatisticaBeanList", estatisticaBeanList);
			} else {
	...
	public void exportarXLS(String dirNomeTemplate, String nomeArqDown, String nomeListaTemplate, List lista)
		...
		if (isIncluiEmbargosDeclaracao()) {
			map.put("embargoDeclaracao", "Sim");
		} else {
			map.put("embargoDeclaracao", "Não");
		}
		map.put("secaoJudiciaria", secaoJudiciaria == null ? "Todas" : secaoJudiciaria);
		map.put("titulo", "ESTATÍSTICA DE PROCESSOS JULGADOS");
		...
		map.put("totGeralJul", totalGeral);
	...
	public List<EstatisticaProcessosJulgadosSubTableBean> ordenarVaras(
			List<EstatisticaProcessosJulgadosSubTableBean> lista) {
		List<Integer> lista1 = new ArrayList<Integer>();

		// pega todos os números das varas e coloca em uma lista para ordenação
		// posterior
		for (EstatisticaProcessosJulgadosSubTableBean o : lista) {
		...
		// vara
		List<EstatisticaProcessosJulgadosSubTableBean> lista3 = new ArrayList<EstatisticaProcessosJulgadosSubTableBean>();
	...
	public void limparFiltros() {
		...
		incluiEmbargosDeclaracao = false;
		dataFim = null;
		dataInicio = null;
	}
	...
	public EstatisticaProcessosJulgadosList getEstatisticaProcessosJulgadosList() {
		return estatisticaProcessosJulgadosList;
	}

	public void setEstatisticaProcessosJulgadosList(EstatisticaProcessosJulgadosList estatisticaProcessosJulgadosList) {
		this.estatisticaProcessosJulgadosList = estatisticaProcessosJulgadosList;
	}
	...
	public List<EstatisticaProcessosJulgadosBean> getEstatisticaBeanList() {
		return estatisticaBeanList;
	}

	public void setEstatisticaBeanList(List<EstatisticaProcessosJulgadosBean> estatisticaBeanList) {
	...
	public static EstatisticaProcessosJulgadosAction instance() {
