...
public class RoutingKeyTest {
    ...
    @Test
    public void testaCriarRoutingKey() throws JsonProcessingException, PJeBusinessException {

        String entityName = ProcessoParte.class.getSimpleName();
        CloudEventVerbEnum evento = CloudEventVerbEnum.POST;
        RoutingKey rk = new RoutingKey("pje-legacy", "403", "2", entityName, evento);


        Assert.assertEquals("403.2.pje-legacy.ProcessoParte.POST",rk.getRoutingKeyValue());

		CloudEvent cloudEvent = CloudEventBuilder.instance()
				.ofPayloadType(ProcessoParteCloudEvent.class)
				.withEntity(this.getProcessoParte())
				.withEvent(CloudEventVerbEnum.POST)
				.build();

		Assert.assertEquals("403.2.pje-legacy.ProcessoParte.POST",cloudEvent.getRoutingKey().toString());
    }
