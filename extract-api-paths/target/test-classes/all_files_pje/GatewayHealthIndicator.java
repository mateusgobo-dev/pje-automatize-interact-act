...
public class GatewayHealthIndicator extends AbstractHealthIndicator{
	...
	public Health doHealthCheck() {
		...
		this.webTarget = this.client.target(getGatewayPath())
									.path(ConfiguracaoIntegracaoCloud.getAppName() + PjeUtil.instance().getContextPath() + "/seam/resource/rest/pje-legacy/status/info");
		...
		try{
			...
			if(resp.getStatus() == HttpStatus.SC_OK){
				this.getDetails().put("success", "O pje-legacy consegue se comunicar com o  serviço gateway corretamente.");
				this.getDetails().put("url", ConfiguracaoIntegracaoCloud.getUrlPje2Gateway());
				this.setHealth(new Health(Status.UP, this.getDetails()));
			} else {
				this.getDetails().put("error", "Problema de comunicação entre o pje-legacy e o serviço gateway.");
				this.getDetails().put("url", ConfiguracaoIntegracaoCloud.getUrlPje2Gateway());
