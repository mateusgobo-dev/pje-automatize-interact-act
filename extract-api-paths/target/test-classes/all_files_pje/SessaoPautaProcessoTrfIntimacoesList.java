...
@Name(SessaoPautaProcessoTrfIntimacoesList.NAME)
@BypassInterceptors
@Scope(ScopeType.CONVERSATION)
public class SessaoPautaProcessoTrfIntimacoesList extends EntityList<SessaoPautaProcessoTrf> {

	public static final String NAME = "sessaoPautaProcessoTrfIntimacoesList";
	...
	private static final String R1 = "proc.numeroSequencia = #{sessaoPautaProcessoTrfIntimacoesList.numeroProcesso.numeroSequencia}";
	private static final String R2 = "proc.numeroDigitoVerificador = #{sessaoPautaProcessoTrfIntimacoesList.numeroProcesso.numeroDigitoVerificador}";
	private static final String R3 = "proc.ano = #{sessaoPautaProcessoTrfIntimacoesList.numeroProcesso.ano}";
	private static final String R4 = "proc.numeroOrigem = #{sessaoPautaProcessoTrfIntimacoesList.numeroProcesso.numeroOrigem}";
	private static final String R5 = "proc.numeroOrgaoJustica = #{sessaoPautaProcessoTrfIntimacoesList.numeroProcesso.numeroOrgaoJustica}";
	private static final String R6 = "proc.orgaoJulgador = #{sessaoPautaProcessoTrfIntimacoesList.orgaoJulgador}";
	...
	private static final String R9 = "proc in (select pp.processoTrf from ProcessoParte pp where pp.processoTrf = proc and "
			+ "lower(to_ascii(pp.pessoa.nome)) like '%' || lower(to_ascii(#{sessaoPautaProcessoTrfIntimacoesList.nomeParte})) || '%')";
	...
	private static final String R12 = "proc.idProcessoTrf in (select c.processo.idProcesso from ProcessoDocumento c where c.processo.idProcesso = proc.idProcessoTrf and "
			+ "c.tipoProcessoDocumento.tipoProcessoDocumento like #{sessaoPautaProcessoTrfIntimacoesList.relatorio ? 'Relatório' : null})";
	private static final String R13 = "o.processoExpediente.sessao.tipoSessao =  #{sessaoPautaProcessoTrfIntimacoesList.tipoSessao}";
	private static final String R14 = "cast(proc.dataDistribuicao as date) >= #{sessaoPautaProcessoTrfIntimacoesList.dtDistribuicaoInicio}";
	private static final String R15 = "cast(proc.dataDistribuicao as date) <= #{sessaoPautaProcessoTrfIntimacoesList.dtDistribuicaoFim}";
	private static final String R16 = "proc.orgaoJulgadorColegiado = #{sessaoPautaProcessoTrfIntimacoesList.orgaoJulgadorColegiado}";
	...
	protected String getDefaultEjbql() {
		...
		sb.append("select distinct o from ProcessoParteExpediente o ");
		...
		sb.append("where o.dtCienciaParte is null ");
	...
	public String getIdProcessoParteExpedienteUsuarioLogado(SessaoPautaProcessoTrf sessaoPautaProcessoTrf) {
		...
		String sql = "select o from ProcessoParteExpediente o "
				+ "where o.dtCienciaParte is null and o.processoJudicial = :processoTrf "
	...
	/**
	 * Método que retorna o modelo de documento do tipo intimação de pauta
	 * 
	 * @param processoTrf
	 *            = ProcessoTrf que é usado no filtro
	 * @return retorna o modelo de documento intimação de pauta
	 */
	public String processoDocumentoIntimacaoPauta(ProcessoTrf processoTrf) {
		String sql = " select o.processoDocumentoBin.modeloDocumento from ProcessoDocumento o where o.processo.idProcesso = :idProcesso "
				+ " and tipoProcessoDocumento.idTipoProcessoDocumento = :idTipoIntimacaoPauta";

		Query query = EntityUtil.createQuery(sql);
		query.setParameter("idProcesso", processoTrf.getProcesso().getIdProcesso());
		query.setParameter("idTipoIntimacaoPauta", ParametroUtil.instance().getTipoProcessoDocumentoIntimacaoPauta()
				.getIdTipoProcessoDocumento());
		EntityUtil.getSingleResult(query);
