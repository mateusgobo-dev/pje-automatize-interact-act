...
public class SorteioVaraCargo2 {

	List<VaraCargo> varaCargoList;

	public SorteioVaraCargo2() {
	...
	public void calcPesos() {
		...
		if (total == 0) {
			...
			for (VaraCargo varaCargo : varaCargoList) {
				varaCargo.setPeso(varaTotalMap.get(varaCargo.getNomeVara()));
			}
		} else {
			float peso = ((float) total / varaCargoList.size()) / (totalClasse / varaCargoList.size());
			for (VaraCargo varaCargo : varaCargoList) {
				varaCargo.setPeso(varaCargo.getQuantidadeClasse() * peso + varaTotalMap.get(varaCargo.getNomeVara()));
			}
		}
	}
